...
class Exit(CoreObject):
    ...
    def getEntity(self):
        ...
        if self.cancelCondition:
            if (
                self.cancelCondition.get("reason", None) == "throughput" and int(self.cancelCondition.get("number", -1)) == self.numOfExits
            ):
                self.endSimulation()
            if (
                self.cancelCondition.get("reason", None) == "empty" and self.checkIfSystemEmpty()
            ):
                self.endSimulation()
        return activeEntity
    ...
    def outputResultsJSON(self):
        ...
        json = {
            "_class": "Dream.%s" % self.__class__.__name__,
            "id": self.id,
            "family": self.family,
            "results": {},
        }
