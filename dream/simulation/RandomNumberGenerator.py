...
class RandomNumberGenerator(object):
    ...
    def __init__(self, obj, distribution):
        ...
        knownDistributions = ['Fixed', 'Normal', 'Exp', 'Gamma', 'Logistic', 'Erlang', 'Geometric', 'Lognormal', 'Weibull', 'Cauchy', 'Triangular']
        for key in distribution.keys():
            if key in knownDistributions:
        ...
        # pop irrelevant keys
        for key in distribution.keys():
            if key not in knownDistributions:
    ...
    def generateNumber(self):
        ...
        if(self.distributionType == "Fixed"):     # if the distribution is Fixed
            ...
            if self.max < self.min:
                raise ValueError("Normal distribution for %s uses wrong parameters. max (%s) > min (%s)" % (self.obj.id, self.max, self.min))
            while 1:
                number = G.Rnd.normalvariate(self.mean, self.stdev)
                # if max=0 this means that we did not have time "time" bounds
                if number > self.max or number < self.min and self.max != 0:  # if the number is out of bounds repeat the process
                    continue
