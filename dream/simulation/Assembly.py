...
class Assembly(CoreObject):
    ...
    def __init__(self, id="", name="", processingTime=None, inputsDict=None, **kw):
        ...
        if (
            "Normal" in list(processingTime.keys()) and processingTime["Normal"].get("max", None) is None
        ):
    ...
    def parseInputs(self, inputsDict):
        ...
        if (
            processingTime["distributionType"] == "Normal" and processingTime.get("max", None) is None
        ):
    ...
    def haveToDispose(self, callerObject=None):
        ...
        return (
            len(activeObjectQueue) > 0 and (thecaller in activeObject.next) and activeObject.waitToDispose
        )
